...
namespace OpenRA.Graphics
{
	public class SpriteRenderer : Renderer.IBatchRenderer
	{
		...
		internal void DrawSprite(Sprite s, float paletteTextureIndex, in float3 location, in float3 scale, in int rotAngle = 0)
		{
			var samplers = SetRenderStateForSprite(s);
			Util.FastCreateQuad(vertices, location + scale * s.Offset, s, samplers, paletteTextureIndex, nv,
								scale * s.Size, float3.Ones, 1f, rotAngle);
			nv += 6;
		}

		internal void DrawSprite(Sprite s, float paletteTextureIndex, in float3 location, float scale, in int rotAngle = 0)
		{
			var samplers = SetRenderStateForSprite(s);
			Util.FastCreateQuad(vertices, location + scale * s.Offset, s, samplers, paletteTextureIndex, nv,
								scale * s.Size, float3.Ones, 1f, rotAngle);
		...
		public void DrawSprite(Sprite s, PaletteReference pal, in float3 location, float scale = 1f, in int rotAngle = 0)
		{
			DrawSprite(s, ResolveTextureIndex(s, pal), location, scale, rotAngle);
		}

		internal void DrawSprite(Sprite s, float paletteTextureIndex, in float3 location,
								 float scale, in float3 tint, float alpha, in int rotAngle = 0)
		{
			var samplers = SetRenderStateForSprite(s);
			Util.FastCreateQuad(vertices, location + scale * s.Offset, s, samplers, paletteTextureIndex, nv,
								scale * s.Size, tint, alpha, rotAngle);
			nv += 6;
		}

		public void DrawSprite(Sprite s, PaletteReference pal, in float3 location, float scale, in float3 tint, float alpha, in int rotAngle = 0)
		{
			DrawSprite(s, ResolveTextureIndex(s, pal), location, scale, tint, alpha, rotAngle);
		}

		internal void DrawSprite(Sprite s, float paletteTextureIndex, in float3 a, in float3 b, in float3 c, in float3 d,
